...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public ResponseVO getMoviePlacingRateByDate(Date date) {
        ...
        try{
            ...
            requireDate = simpleDateFormat.parse(simpleDateFormat.format(requireDate));
            Date nextDate = getNumDayAfterDate(requireDate, 1);
            List<PlacingRate> placingRateList = statisticsMapper.selectPlacingRate(requireDate,nextDate);
            for(PlacingRate pRate:placingRateList){
                placingRateVOList.add(new PlacingRateVO(pRate));
            }
    ...
    public ResponseVO getPopularMovies(int days, int movieNum) {
        try{
            if(movieNum<=0||days<0){
                return ResponseVO.buildFailure("参数不合要求");
            }
            List<MovieBoxOfficeVO> popularMovies = new ArrayList<>();
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
            Date today = simpleDateFormat.parse(simpleDateFormat.format(new Date()));
            Date startDate = getNumDayAfterDate(today, 1-days);
            List<MovieBoxOffice> recentDaysBoxOffice = statisticsMapper.selectMovieBoxOffice(startDate,today);
            for(int i =0;i<movieNum&&i<recentDaysBoxOffice.size();i++){
                popularMovies.add(new MovieBoxOfficeVO(recentDaysBoxOffice.get(i)));
            }
